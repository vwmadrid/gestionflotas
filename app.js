...
// ============================================================================
// 🖨️ / 💬 GENERADOR DE HOJAS Y ENVIADOR DE WHATSAPP PARA PREPARADORES (V2)
// ============================================================================
...
window.generarListadoDiario = function() {
    ...
    // 2. Preguntamos para qué día queremos el listado
    Swal.fire({
        ...
        if (result.isConfirmed && result.value) {
            let fechaElegida = result.value; 
            let fechaVisual = fechaElegida.split('-').reverse().join('/');

            // Filter de citas del día (excluyendo "---" y bloqueos)
            ...
            // Ordenamos cronológicamente por hora de entrega
            citasDelDia.sort((a, b) => a.fechaHora.localeCompare(b.fechaHora));

            // 3. SEGUNDO PASO: Preguntar al usuario qué canal quiere usar
            Swal.fire({
                title: '¿Qué deseas hacer?',
                text: `Hay ${citasDelDia.length} entregas planificadas para el ${fechaVisual}.`,
                icon: 'question',
                showCancelButton: true,
                showDenyButton: true,
                confirmButtonColor: '#00b0f0', // Azul claro para imprimir
                denyButtonColor: '#25D366',    // Verde WhatsApp
                cancelButtonColor: '#6b7280',
                confirmButtonText: '<i class="ph-bold ph-printer"></i> Imprimir Papel',
                denyButtonText: '<i class="ph-bold ph-whatsapp-logo"></i> Enviar por WhatsApp',
                cancelButtonText: 'Cancelar'
            }).then((canalResult) => {
                
                // ==========================================
                // OPCCIÓN A: IMPRIMIR HOJA CORPORATIVA
                // ==========================================
                if (canalResult.isConfirmed) {
                    let htmlImprimir = `
                        <!DOCTYPE html>
                        <html lang="es">
                        <head>
                            ...
                            <style>
                                ...
                                @media print { .btn-print { display: none !important; } body { margin: 0; } @page { margin: 1cm; size: A4 portrait; } }
                            </style>
                        </head>
                        <body>
                            <div class="cabecera">
                                        ...
                                        return `
                                            <tr>
                                                ...
                                                <td style="font-size: 12px; color: #ef4444; font-weight: bold;">
                                                    ${etiquetaVO}
                                                    <div style="margin-top: ${etiquetaVO ? '6px' : '0'};">${c.notes || c.notas || ''}</div>
                                                </td>
                ...
                // ==========================================
                // OPCIÓN B: REDACTAR Y ENVIAR POR WHATSAPP
                // ==========================================
                } else if (canalResult.isDenied) {
                    
                    // Construimos el mensaje de texto estructurado
                    let msgWhatsApp = `📋 *PLANING DE PREPARACIÓN Y LAVADERO* \n📅 *Día:* ${fechaVisual}\n`;
                    msgWhatsApp += `=============================\n\n`;

                    citasDelDia.forEach(c => {
                        let horaCita = c.fechaHora.split('T')[1].substring(0, 5);
                        let tieneVO = (c.entregaVO === 'SÍ' || c.entregaVO === 'SI') ? ' ⚠️ *ENTREGA V.O.*' : '';
                        let notaLimpia = c.notes || c.notas ? ` \n   └ 📝 *Notas:* _${c.notes || c.notas}_` : '';

                        msgWhatsApp += `⏰ *${horaCita}h* - *${c.matricula}*\n`;
                        msgWhatsApp += `   🚗 ${c.modelo.toUpperCase()}\n`;
                        msgWhatsApp += `   👤 *Asesor:* ${c.agente}${tieneVO}${notaLimpia}\n`;
                        msgWhatsApp += `--------------------------------------\n`;
                    });

                    msgWhatsApp += `\n_Por favor, tened listos los vehículos limpios con un margen de 30 minutos antes de su hora. ¡Gracias equipo!_`;

                    // Lanzamos la URL universal de WhatsApp (wa.me sin número abre la lista de tus chats recientes para elegir grupo)
                    let urlEnvio = `https://wa.me/?text=${encodeURIComponent(msgWhatsApp)}`;
